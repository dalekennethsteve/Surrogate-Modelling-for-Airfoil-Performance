...
scaler_cl = joblib.load('scaler_cl.pkl')
scaler_cd = joblib.load('scaler_cd.pkl')
gpr_cl = joblib.load('gpr_cl.pkl')
gpr_cd = joblib.load('gpr_cd.pkl')
...
def predict_gpr(alphas):
    """
    Predict CL and CD for multiple alpha values
    """
    ...
    # CL predictions
    cl_scaled = gpr_cl.predict(X_scaled)
    cl = scaler_cl.inverse_transform(cl_scaled.reshape(-1, 1)).ravel()

    # CD predictions (log scale)
    cd_scaled = gpr_cd.predict(X_scaled)
    cd_log = scaler_cd.inverse_transform(cd_scaled.reshape(-1, 1)).ravel()
    cd = np.exp(cd_log)
...
cl_values, cd_values = predict_gpr(alphas)
