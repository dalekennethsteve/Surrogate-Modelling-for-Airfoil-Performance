...
# Load
rf_cl = joblib.load('rf_cl.pkl')
rf_cd = joblib.load('rf_cd.pkl')
...
def predict_rf(alphas):
    """
    Predict CL and CD for multiple alpha values using Random Forest
    """
    ...
    cl = rf_cl.predict(data)
    cd = rf_cd.predict(data)
...
cl_values, cd_values = predict_rf(alphas)
